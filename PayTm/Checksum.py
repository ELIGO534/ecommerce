...
def generate_checksum(param_dict, merchant_key, salt=None):
    ...
    if isinstance(merchant_key, str):
        merchant_key = merchant_key.encode()
...
def __encode__(to_encode, iv, key):
    print(f"Key: {key}, Type: {type(key)}, Length: {len(key)}")
    print(f"IV: {iv}, Type: {type(iv)}, Length: {len(iv)}")
    # Pad
    to_encode = __pad__(to_encode)  
    
    # Encrypt
    if isinstance(to_encode, str):  # Ensure it's a string before encoding
        to_encode = to_encode.encode('utf-8')
    
    c = AES.new(
    key if isinstance(key, bytes) else key.encode('utf-8'),
    AES.MODE_CBC,
    iv if isinstance(iv, bytes) else iv.encode('utf-8')
)
    ...
    return to_encode.decode('utf-8')  # Return as string
